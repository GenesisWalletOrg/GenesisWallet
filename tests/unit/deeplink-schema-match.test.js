...
describe.each(['', '//'])('unit - DeepLinkSchemaMatch', function (suffix) {
  ...
  it('navigationForRoute', async () => {
    const events = [
      ...
      {
        argument: {
          url: 'bluewallet:setelectrumserver?server=infra1.bitcoin-ii.org%3A443%3As',
        },
        expected: [
          'ElectrumSettings',
          {
            server: 'infra1.bitcoin-ii.org:443:s',
          },
        ],
      },
      {
  ...
  it('can work with some deeplink actions', () => {
    ...
    assert.strictEqual(
      DeeplinkSchemaMatch.getServerFromSetElectrumServerAction('bluewallet:setelectrumserver?server=infra1.bitcoin-ii.org%3A443%3As'),
      'infra1.bitcoin-ii.org:443:s',
    );
    assert.strictEqual(
      DeeplinkSchemaMatch.getServerFromSetElectrumServerAction('setelectrumserver?server=infra1.bitcoin-ii.org%3A443%3As'),
      'infra1.bitcoin-ii.org:443:s',
    );
    assert.strictEqual(
      DeeplinkSchemaMatch.getServerFromSetElectrumServerAction('ololo:setelectrumserver?server=infra1.bitcoin-ii.org%3A443%3As'),
      false,
    );
    assert.strictEqual(
      DeeplinkSchemaMatch.getServerFromSetElectrumServerAction('setTrololo?server=infra1.bitcoin-ii.org%3A443%3As'),
      false,
    );
